<?php 
	...
	if (empty($_POST['log_name'])) $msg = "pls enter valid name";
		...
		$str_chk="select count(*) as cnt from sms_users where log_email = '$email'";
		// echo($str_chk);
		...
		if($final_cnt['cnt']>0){
			...
			$str="insert into sms_users(log_name,log_mobile,log_email,log_password) values ('$name','$mobile','$email','$password')";
